...
{
    const int startStdDev = 5;  // Zero based index, first sample used for std deviation calculation. Skip first 5 measurements to let system settle down.
    const int endStdDev = 15;   // Zero based index, last sample used for std deviation calculation
    const int neededSteps = 7; // Number of continous samples over the threshold needed to determine a hit
...
if (ndataPoints>endStdDev)
{
    ...
    for (int j=0; j<nsamps-1; j++)
    {
        ...
        for (int i=0; i<nsamps; i++) // Looping through the existing data file for 3 samples compared to control. Needs to be done for real time data with 3 samples and the control. The samples are triplicate repeats of the same test. Remove from final product.
        {
            ...
            {
                double value = data[j][i]-data[j-3][i];
                if (value > 0.02*data[j][i])
                {
                    ++ counter;
                    if (counter == 5) // Make sure the positive slope persists for at least 5 occurrences.
                    {
                        firstTime = j-5;
                        bar[i][1]=firstTime;
                        bar[i][2]=ndataPoints;
